class SetProductCook
  include ExtendedInteractor

  def call
    after_check do
      failures = []
      operation_record = Operation.find_by_operation_name("Set_Product_Cook")
      context[:products].each do |product|
        product_data = ProductData.find_by(product_id: product.id)
        constructParams(product, product_data.cook_data)
  ...
  def constructParams(product, cook_data)
    context[:params] = []
    context[:params] << "{pWonum};" + product.work_order_id.to_s
    cook_data = JSON.parse(cook_data)
    cook_data.each do |key, value|
